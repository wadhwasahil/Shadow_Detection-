...
def default_conv(input, num_filters):
    ...
    W = tf.get_variable('weights' + str(num_filters), [5, 5, input_channels, num_filters], initializer=tf.random_normal_initializer(stddev=0.2))
    conv = tf.nn.conv2d(input, filter=W, strides=[1, 2, 2, 1], padding="SAME", name="conv") + b
    return conv
...
class Discriminator(object):
    def __init__(self, rgb_image, shadow_mask, reuse=False):
        with tf.variable_scope("discriminator"):
            ...
            self.stack = tf.concat([rgb_image, self.shadow_mask_expanded], axis=3)
